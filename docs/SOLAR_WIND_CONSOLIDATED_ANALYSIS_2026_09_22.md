...
**Later image/source audit:** see [NISE Kerala annexure and World Bank exclusion-map inspection, with the NIWE visual-review gap explicitly marked](SOLAR_WIND_VISUAL_SOURCE_CROSSWALK_2026_09_22.md). The original consolidated analysis below retains its earlier assumptions unless explicitly superseded by this later evidence.
...
6. **The five extracted source-folder snapshots are archived and independently SHA-256-restored in a verified PRIVATE repository** ([asset ledger](../data/evidence/solar/renewable_five_folder_private_archive_2026_09_22.json)). The separate 17-file exact-original publisher ZIP/PDF check remains OPEN; a repacked extracted folder does not prove byte-identical restoration of every originally downloaded container.
...
- **Keep archive distinctions explicit**: all five extracted source-folder snapshots are safely uploaded to a private GitHub Release and independently restored through member SHA-256 checks. The 17 byte-identical historical publisher-original ZIP/PDF stream comparison remains OPEN as a separate status; archive raw third-party bytes only in access-controlled storage consistent with publisher terms.

**Repository gate:** FY solar weather validation **FAIL**; **five-folder private archival PASS**, exact 17 provider-original byte-identical archival OPEN; descriptive exact Kerala onshore resource clips and full-India GSA PVOUT unit QA have passed, but eligible wind/PV MW and independently validated hourly generation remain **OPEN**.
