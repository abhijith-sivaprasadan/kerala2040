...
The package IDs and formats are already audited from a preserved raw snapshot of
KSEB's own WordPress Download Manager listing. This downloader performs no URL
or filename discovery: it executes that fixed inventory, validates file magic,
records final response metadata and SHA-256, and fails closed on any missing or
wrong-format month.
"""
    ...
    for attempt in range(1, attempts + 1):
        for url in urls:
            ...
            try:
                ...
                errors.append(f"attempt={attempt} url={url}: {type(exc).__name__}: {exc}")
        if attempt < attempts:
