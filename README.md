...
Two new 24-hour **synthetic, service-preserving** WP6 experiments compare immediate vs managed charging for three illustrative EVs and earliest vs scheduled auxiliary demand for three **fictional** noncritical industrial jobs. Each policy must deliver exactly the same battery-side charge or process duty before identical departure/deadline times, obey unchanged individual and shared kW limits, and preserve all nonshiftable industrial safety/critical loads. Nine independently rerun physical sensitivity pairs per new pilot (18 total) are published with both hourly profiles, service checks, evening/whole-day peaks, energy, model and input files. **The industrial jobs are NOT verified KMML dispatch permissions**, and neither experiment is measured Kerala feeder demand, annual benefit, transport electrification or 2040 optimisation. Together with cooling/TES, WP6 now has three bounded executable pilots; BESS, pumped-storage and validated site/grid scaling remain separate work.

## WP6 cooling and thermal storage · executable first experiment · 24 September 2026

[Technical methods & limitations](docs/WP6_COOLING_THERMAL_STORAGE_PILOT_2026_09_24.md) · [Explicit synthetic assumptions](configs/wp6_cooling_tes_illustrative.yaml) · [1R1C model source](src/kerala2040/flexibility_cooling.py) · [interactive Pathways study](https://kerala2040.github.io/#pathways).

Three deterministic 24-hour experiments now compare conventional AC, early thermal-mass pre-cooling and an independently charged 6 kWh_th cold-water store for a **synthetic one-zone** thermal system under the **same 24–26°C comfort band and terminal room/store states**. Results report **whole-day electric peak separately from illustrative 17–21 peak**, total and evening grid kWh, stored cold-energy balances, early-target shortfall vs actual comfort violation, and nine capacity×charge-COP sensitivity cases. All hourly results are regenerated during the website build; model, config, CLI, tests and dynamic chart/data tables are public. This is **not actual Kerala weather, cooling energy, feeder/grid coincidence, annual rupees/CO₂, grid-service capacity, or solved 2040 optimisation**. The rest of WP6 (EV smart charging, BESS, pumped storage and industrial flexibility) remains open.
